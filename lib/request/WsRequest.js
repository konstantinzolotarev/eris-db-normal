...
module.exports = class WsRequest extends Request {
  ...
  _connect () {
    ...
    this._ws.on('open', () => {
      // WS opned
      console.log('Opened connection') // eslint-disable-line
    })
  ...
  onClose () {
    // Ws Closed
    console.log('Closed connection') // eslint-disable-line
  }
  ...
  onMessage (message) {
    ...
    try {
      ...
      if (!_.isObject(json))
        return
      // Might be event from blockchain
      if (!json.id || !_.isString(json.id))
        return

      if (this._subscriptions[json.id] || !this._promises[json.id]) {
        this
          .transformResponse(json)
          .then((data) => {
            this.emit('event', {
              event: this._subscriptions[json.id],
              subId: json.id,
              data: data
            })
          })
  ...
  call (method, params) {
    ...
    if (method == 'eventUnsubscribe')
      this.removeSubscription(params.sub_id)
